/**
 * CADDY v0.4.0 - Workflow Node Component
 * Reusable workflow node with drag-and-drop support
 */

import React, { useCallback, useMemo, useRef, useState } from 'react';
import { useDrag } from 'react-dnd';
import type { WorkflowNode as WorkflowNodeType, NodePort, ExecutionStatus } from './types';
...
export const WorkflowNode: React.FC<WorkflowNodeProps> = ({
  ...
  const handlePortMouseDown = useCallback(
    (port: NodePort, e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  return (
    ...
    >
      ...
      {/* Body */}
      <div style={bodyStyle}>
        {node.data.description && <div style={{ marginBottom: '4px' }}>{node.data.description}</div>}
      ...
      {/* Progress Bar */}
      {isExecuting && executionProgress > 0 && <div style={progressBarStyle} />}
    </div>
  );
};
