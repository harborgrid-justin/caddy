/**
 * CADDY Enterprise Dashboard Module v0.4.0
 *
 * Comprehensive enterprise dashboard system with real-time metrics, KPIs,
 * interactive charts, customizable widgets, executive overview, and export capabilities.
 *
 * @module dashboard
 */
...
// Filters
export {
  DashboardFiltersComponent as DashboardFilters,
...
/**
 * Utility: Create dashboard configuration
 */
export function createDashboardConfig(
  overrides: Partial<typeof DEFAULT_DASHBOARD_CONFIG> = {}
) {
  return {
...
export function calculateTrend(
  current: number,
  previous: number
): { direction: 'up' | 'down' | 'neutral'; change: number; changePercent: number } {
  if (previous === 0) {
    return {
...
/**
 * Utility: Get time range dates
 */
export function getTimeRangeDates(timeRange: string): { start: Date; end: Date } {
  const end = new Date();
...
/**
 * Utility: Aggregate data by interval
 */
...
/**
 * Utility: Generate mock metric data (for testing)
 */
export function generateMockMetricData(count: number = 10) {
...
/**
 * Utility: Generate mock chart data (for testing)
 */
export function generateMockChartData(type: 'line' | 'bar' | 'pie' = 'line', points: number = 10) {
