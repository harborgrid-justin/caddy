/**
 * CADDY v0.4.0 - Report Builder Component
 * $650M Platform - Production Ready
 *
 * Comprehensive drag-and-drop report designer with WYSIWYG capabilities,
...
export const ReportBuilder: React.FC<ReportBuilderProps> = ({
  ...
  // Initialize definition with defaults
  function initializeDefinition(partial?: Partial<ReportDefinition>): ReportDefinition {
    const now = new Date();
    return {
      ...
      dataSource: partial?.dataSource || dataSources[0] || {} as DataSource,
      query: partial?.query || {
        select: [],
        from: '',
      },
  ...
  // Update definition with history tracking
  const updateDefinition = useCallback(
    (updater: (def: ReportDefinition) => ReportDefinition) => {
      setState((prev) => {
  ...
  const addSection = useCallback(
    (type: ReportSection['type'], position?: { x: number; y: number }) => {
  ...
  const updateSection = useCallback(
    (sectionId: string, updates: Partial<ReportSection>) => {
      updateDefinition((def) => ({
        ...
        layout: {
          ...def.layout,
          sections: def.layout.sections.map((section) =>
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      if (draggedItem.section) {
        ...
        // Adding new section
        addSection(draggedItem.type as ReportSection['type'], { x, y });
      }
    ...
    <div style={styles.container}>
      ...
      <div style={styles.toolbar}>
        <div style={styles.toolbarLeft}>
          <input
            ...
            onChange={(e) =>
              updateDefinition((def) => ({ ...def, name: e.target.value }))
            }
            style={styles.titleInput}
            disabled={readOnly}
            placeholder="Report Name"
          />
          <span style={styles.versionBadge}>v{state.definition.version}</span>
          <span style={styles.statusBadge}>{state.definition.status}</span>
        </div>

        <div style={styles.toolbarRight}>
          <button
            onClick={undo}
            disabled={readOnly || state.history.past.length === 0}
          ...
          <button
            onClick={redo}
            disabled={readOnly || state.history.future.length === 0}
            style={styles.toolbarButton}
            title="Redo (Ctrl+Y)"
          >
            ↷ Redo
          ...
          <button onClick={togglePreview} style={styles.toolbarButton}>
            {state.previewMode ? '✏️ Edit' : '👁️ Preview'}
          </button>

          <div style={styles.separator} />

          <button onClick={validate} style={styles.toolbarButton}>
            ✓ Validate
          </button>
          <button
            onClick={handleSave}
            disabled={readOnly || saving || !state.isDirty}
            style={{
          ...
          {onCancel && (
            <button onClick={onCancel} style={styles.toolbarButton}>
              ✕ Cancel
            </button>
          )}
        </div>
        ...
        <div style={styles.validationPanel}>
          {validation.errors.map((error, index) => (
            <div key={index} style={styles.errorMessage}>
              ⚠️ {error.field}: {error.message}
            </div>
          ))}
          {validation.warnings.map((warning, index) => (
            <div key={index} style={styles.warningMessage}>
              ℹ️ {warning.field}: {warning.message}
            </div>
          ))}
        </div>
      )}
      ...
      <div style={styles.content}>
            ...
            <div style={styles.paletteGrid}>
              {[
                ...
                <div
                  key={component.type}
                  draggable
                  onDragStart={(e) => handleDragStart(e, { type: component.type })}
                  style={styles.paletteItem}
                >
                  <span style={styles.paletteIcon}>{component.icon}</span>
                  <span style={styles.paletteLabel}>{component.label}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={styles.canvasContainer}>
          ...
          >
            {state.definition.layout.sections.map((section) => (
              <div
                key={section.id}
                draggable={!state.previewMode && !readOnly}
                onDragStart={(e) =>
                  handleDragStart(e, { type: 'section', section, index: 0 })
                }
                onClick={() => !state.previewMode && setSelectedSection(section.id)}
                style={{
                  ...styles.section,
                  ...section.style,
        ...
        {/* Properties Panel */}
        {selectedSection && !state.previewMode && (
          <div style={styles.propertiesPanel}>
            <h3 style={styles.propertiesPanelTitle}>Properties</h3>
            {renderProperties(
              state.definition.layout.sections.find((s) => s.id === selectedSection)!,
              updateSection
            )}
          </div>
        )}
...
function getDefaultContent(type: ReportSection['type']): any {
  switch (type) {
    case 'header':
...
function renderSectionContent(section: ReportSection): React.ReactNode {
  switch (section.type) {
    case 'header':
    case 'footer':
    case 'text':
      return <div>{section.content?.text || 'Empty'}</div>;
    case 'table':
      return <div>Table: {section.content?.columns?.length || 0} columns</div>;
    case 'chart':
      return <div>Chart: {section.content?.type || 'bar'}</div>;
    case 'image':
      return <div>Image: {section.content?.url || 'No URL'}</div>;
    default:
