 ...
 * CADDY API Testing
 *
 * API testing interface with test suites, automated tests, and result visualization.
 */
...
export const APITesting: React.FC<APITestingProps> = ({ onRunTest, onRunSuite }) => {
  ...
  const getSuitePassRate = (suite: TestSuite): number => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Suites</div>
            <div className="text-3xl font-bold text-gray-900 dark:text-white">{suites.length}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Tests</div>
            <div className="text-3xl font-bold text-gray-900 dark:text-white">
              {suites.reduce((sum, s) => sum + s.tests.length, 0)}
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {suites.map((suite) => (
              ...
              >
                <div
                  ...
                  className={`px-6 py-4 cursor-pointer border-b border-gray-200 dark:border-gray-700 ${
                    selectedSuite?.id === suite.id ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                  }`}
                >
                  ...
                  <div className="flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    {getSuitePassRate(suite) > 0 && (
                      <span className="text-green-600 dark:text-green-400">
                        {getSuitePassRate(suite).toFixed(0)}% pass rate
                      </span>
                    )}
                  </div>
                </div>
                            ...
                            <div className="flex-1">
                              ...
                              {result && (
                                <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                  Duration: {result.duration}ms • {result.assertions.length}{' '}
                                  assertion(s)
                                </div>
                              )}
                            </div>
          ...
          <div className="lg:col-span-1">
            {selectedTest && testResults[selectedTest.id] ? (
                ...
                <div className="p-6 space-y-4 max-h-[600px] overflow-y-auto">
                  <div
                    className={`p-4 rounded-lg ${
                      testResults[selectedTest.id].passed
                  ...
                  >
                    <div className="text-center">
                      ...
                      >
                        {testResults[selectedTest.id].passed ? 'PASSED' : 'FAILED'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {testResults[selectedTest.id].assertions.map((assertion, index) => (
                        <div
                          key={index}
                          ...
                          <div className="flex items-start space-x-2">
                            ...
                            <div className="flex-1 text-sm">
                              <div className="font-medium text-gray-900 dark:text-white">
                                {assertion.type}: {assertion.operator}
                              </div>
                  ...
                  {testResults[selectedTest.id].response && (
                    <div>
                      <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                        Response
                      </h4>
                      <div className="bg-gray-900 text-gray-100 p-3 rounded text-xs overflow-x-auto">
                        <pre>
                          {JSON.stringify(testResults[selectedTest.id].response, null, 2)}
                        </pre>
                      </div>
                    </div>
                  )}
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 text-center text-gray-500 dark:text-gray-400 sticky top-4">
