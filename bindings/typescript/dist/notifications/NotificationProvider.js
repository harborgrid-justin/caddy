...
import { NotificationStatus, NotificationType, NotificationPriority, NotificationChannel, NotificationGroupBy } from './types';
export const NotificationContext = createContext(null);
export const NotificationProvider = ({ children, apiUrl = '/api/notifications', wsUrl = '/ws/notifications', tenantId, userId, autoConnect = true, pollInterval = 30000 }) => {
    ...
    const calculateStats = useCallback((notifs) => {
        const now = new Date();
    ...
    const fetchNotifications = useCallback(async (customFilter) => {
        setLoading(true);
        setError(null);
        try {
    ...
    const markAsRead = useCallback(async (id) => {
        ...
        try {
            ...
            setNotifications(prev => prev.map(notif => ids.includes(notif.id)
            ...
            switch (wsEvent.type) {
                case 'notification.created':
                    setNotifications(prev => {
                        const notification = wsEvent.data;
                ...
                case 'notification.updated':
                    setNotifications(prev => prev.map(notif => notif.id === wsEvent.data.id ? wsEvent.data : notif));
                    break;
                case 'notification.deleted':
                case 'notification.archived':
                    setNotifications(prev => prev.filter(notif => notif.id !== wsEvent.data.id));
                    break;
                case 'notification.read':
                    setNotifications(prev => prev.map(notif => notif.id === wsEvent.data.id
                        ? { ...notif, status: NotificationStatus.READ, readAt: new Date() }
                        : notif));
                    break;
            }
